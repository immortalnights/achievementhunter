...
module.exports = function(config) {
	...
	router.route('/Games/:id/Resynchronize/invoke')
		.put((req, res) => {
			...
			debug("Received game resynchronization request", gameId);
			req.notifier.emit('resynchronize', { resource: 'game', id: gameId });

			if (playerId)
			{
				req.notifier.emit('resynchronize', { resource: 'player_achievements', id: playerId, gameId: gameId })
			}
