...
module.exports = class Player {
	...
	async resynchronizeProfile()
	{
		try
		{
			...
			if (friends)
			{
				...
				profile.friends = _.map(friends, function(friend) {
					return _.pick(friend, 'steamid', 'avatar', 'friend_since');
				});
				debug(`Player '${this.name}' has ${friends.length} friends`);
			}
		...
		{
			console.error(`Failed to resynchronize player profile '${this.name}' (${this.id})`);
			console.error(err);
		}
	}
	...
	async loadPlayedGames()
	{
		...
		{
			console.error(`Failed to resynchronize player games '${this.name}' (${this.id})`);
			console.error(err);
		}
	...
	async updatePlayerAchievementsForGame(gameId)
	{
		...
		try
		{
			...
			if (game.getPlayTimeForPlayer(this.id) !== 0)
			{
				...
				{
					let unlockedCount = 0;
					debug(`Updating ${achievements.length} achievements for player ${this.name}`)
					for (let index = 0; index < achievements.length; index++)
					{
						...
						if (achievement.achieved)
						{
							...
							await database.instance.setGameAchievementAchieved(game.id, this.id, achievement.apiname, unlocktime);
						}
					}

					debug(`Player '${this.name}' has unlocked ${unlockedCount} achievement(s) in '${game.name} (${game.id}`);
