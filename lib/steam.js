...
module.exports = class Steam {
	...
	getOwnedGames(userId)
	{
		...
		return request('IPlayerService/GetOwnedGames/v0001/', {
			...
			{
				console.error(`${moment().toISOString()} Empty response received for player ${userId} games`);
			}
			else if (_.isEmpty(response.games))
			{
				console.warn(`${moment().toISOString()} Empty games list received for player ${userId} games`);
			}
